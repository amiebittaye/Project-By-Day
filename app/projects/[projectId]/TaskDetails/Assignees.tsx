...
import { IUserLink } from '@/utils/users';
...
export const Assignees = () => {
  ...
  useEffect(() => {
    if (task?.assignees) {
      setSelectedAssignees(task.assignees.map((a: { id: any; }) => a.id));
    }
  }, [task?.assignees]);
  ...
  const allMembers = useMemo(() => {
    ...
    // Narrow the owner value to the expected shape so TS knows the properties exist
    const ownerObj = owner as unknown as {
      id: string;
      name: string;
      email?: string;
      avatar?: string;
    ...
    const isOwnerInMembers = members.some((m) => m.id === ownerObj.id);
    ...
    return [
      {
        ...
        description: ownerObj.description,
        links: ownerObj.links,
      },
      ...members,
    ];
  }, [members, owner]);
  ...
  const handlePopoverOpenChange = async (open: boolean) => {
    if (
      ...
      JSON.stringify(selectedAssignees.sort()) !==
        JSON.stringify(task?.assignees?.map((a: { id: any; }) => a.id).sort())
    ) {
      const currentAssignees = task?.assignees?.map((a: { id: any; }) => a.id) || [];
