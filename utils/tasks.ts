...
interface TaskLabelJunction {
  labels?: Label | null;
}

interface TaskAssigneeJunction {
  users?: User | null;
}

/**
 * Minimal ITask shape used throughout this file.
 * Index signature allows passing through other DB fields without error.
 */
interface ITask {
  id: string;
  title?: string | null;
  ...
  task_labels?: TaskLabelJunction[] | null;
  task_assignees?: TaskAssigneeJunction[] | null;
  labels?: Label[] | null;
  assignees?: User[] | null;
...
interface ITaskWithOptions extends ITask {
  labels: Label[];
  assignees: User[];
}
