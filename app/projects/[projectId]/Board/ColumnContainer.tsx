...
// Local type definitions to avoid missing '@/types' module resolution
type IStatus = {
  id: string;
  label: string;
  color?: string;
...
export const ColumnContainer = ({
  ...
  const handleAddItem = async () => {
    ...
    try {
      ...
      onTaskCreated?.({
        ...
        statusPosition: task.statusPosition ?? undefined,
        assignees: [],
      });
      setInputValue('');
      setShowInput(false);
  ...
  return (
    <div
      ref={setNodeRef}
      className="w-87.5 overflow-x-hidden h-full shrink-0 bg-gray-100 dark:bg-gray-950 rounded-md border border-gray-200 dark:border-gray-800 flex flex-col"
    >
              ...
              <Tooltip>
                ...
                {(column.limit ?? 0) > 0 && columnTasks.length >= (column.limit ?? 0) && (
                  <TooltipContent>
                    <p>
                      Column limit{' '}
                      {columnTasks.length > (column.limit ?? 0)
                        ? 'exceeded'
                        : 'reached'}
                    </p>
                  </TooltipContent>
