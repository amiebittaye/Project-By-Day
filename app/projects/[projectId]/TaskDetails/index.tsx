...
import { ActivityResponse, useActivityQueries } from '@/hooks/useActivityQueries';
...
import { CommentResponse } from '@/utils/comments';
...
const MemoizedTimelineItem = React.memo(
  ({
    ...
    return (
      <div className="bg-white dark:bg-gray-950 my-6 -ml-8">
...
export const TaskDetails = () => {
  ...
  const allMembers = useMemo(() => {
    ...
    const isCurrentUserMember = members.some((m: { id: any; }) => m.id === user.id);
    return isCurrentUserMember ? members : [...members, user];
  }, [members, user]);

  // Memoize timeline items
  const timelineItems = useMemo(() => {
    const activitiesArray: ActivityResponse[] = Array.isArray(taskActivities)
      ? (taskActivities as ActivityResponse[])
      : [];
    const commentsArray: CommentResponse[] = Array.isArray(taskComments)
      ? (taskComments as CommentResponse[])
      : [];
    return getTimelineItems(activitiesArray, commentsArray);
  ...
  return (
    <div className="flex flex-col lg:flex-row py-6 gap-6">
      ...
      <div className="w-full lg:w-63.75">
        <Assignees />
        <Separator />
        <TaskLabels />
        <Separator />
