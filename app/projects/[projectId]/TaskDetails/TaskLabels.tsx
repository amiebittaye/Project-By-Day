...
export const TaskLabels = () => {
  ...
  useEffect(() => {
    if (task?.labels) {
      setSelectedLabels(task.labels.map((label: { id: any; }) => label.id));
      // Normalize labels to ensure color is a string (matches expected type)
      const normalizedLabels = task.labels.map((label: any) => ({
        id: String(label.id),
        label: label.label ?? '',
        color: label.color ?? '',
      }));
      updateTaskLabels?.(selectedTask?.id || '', normalizedLabels);
    }
  ...
  const handlePopoverOpenChange = async (open: boolean) => {
    if (
      ...
      JSON.stringify(selectedLabels.sort()) !==
        JSON.stringify(task?.labels?.map((label: { id: any; }) => label.id).sort())
    ) {
      const currentLabels = task?.labels?.map((label: { id: any; }) => label.id) || [];
      ...
      const removedLabels = currentLabels.filter(
        (id: string) => !newLabels.includes(id)
      );
  ...
  const filteredLabels = labels?.filter((label: { label: string; }) =>
