...
export const Project = () => {
  ...
  const calculateNewStatusPosition = (
    ...
    const getPriorityOrder = (task: ITaskWithOptions): number => {
      const p: any = task.priority;
      if (!p) return 0;
      if (typeof p === 'string') return 0;
      return (p?.order as number) ?? 0;
    };
    ...
    const tasksWithSameOrHigherPriority = columnTasks.filter(
      (task) => getPriorityOrder(task) >= newPriorityOrder
    );
    ...
    const tasksWithNextLowerPriority = columnTasks.filter(
      (task) => getPriorityOrder(task) === newPriorityOrder - 1
    );

    if (tasksWithSameOrHigherPriority.length === 0) {
      // No tasks with same or higher priority, place at top
      const maxPos = columnTasks.length
        ? Math.max(...columnTasks.map((t) => t.statusPosition ?? 0))
        : 0;
      return maxPos + 100;
    }
    ...
    const maxPosDefault = columnTasks.length
      ? Math.max(...columnTasks.map((t) => t.statusPosition ?? 0))
      : 0;
    return maxPosDefault + 100;
  };
          ...
          <DropdownMenuContent className="mr-4">
            ...
            {sortedPriorities?.map((priority) => (
              ...
              >
                ...
                />
                <div className="grow">{priority.label}</div>
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
        </DropdownMenu>
          ...
          <DropdownMenuContent className="mr-4">
            ...
            {sortedSizes?.map((size) => (
              ...
              >
                ...
                />
                <div className="grow">{size.label}</div>
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
        </DropdownMenu>
