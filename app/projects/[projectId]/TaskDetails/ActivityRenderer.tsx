...
import { IStatus } from '../ProjectDetails';
  ...
  return (
    <div className="flex items-center flex-wrap text-xs gap-1 ml-3 my-3">
      {activity.content.map((item: any, index: number) => {
        ...
        // ensure item is an object with a 'type' property before accessing it
        ...
        switch (typed.type) {
          case 'user':
            return (
              <User
                key={index}
                user={allMembers?.find((member) => member.id === typed.id)}
              />
            );
          case 'status':
            return (
              <StatusBadge
                ...
                status={statuses?.find(
                  (status: IStatus) => status.id === typed.id
                )}
              />
            );
          case 'label':
            return (
              <LabelRenderer
                key={index}
                label={labels?.find((label: ILabel) => label.id === typed.id)}
              />
            );
          case 'labels':
            return (
              <LabelsRenderer
                key={index}
                labels={typed.ids
                  .map((id: string) => labels?.find((label: ILabel) => label.id === id))
                  ?.filter((label: ILabel | undefined): label is ILabel => label !== undefined)}
              />
            );
          case 'date':
            return <DateRenderer key={index} value={typed.value} />;
          case 'users':
            return (
              <Users
                key={index}
                users={typed.ids
                  ...
                  .filter((member: IUser | undefined): member is IUser => member !== undefined)}
              />
            );
          default:
            return null;
